class MatrixMixin:
    ...
    async def create_new_matrix_channel(self, name, topic, is_public=True):
        ...
        if url:
            await self.opsdroid.send(RoomImage(Image(url=url), target=matrix_room_id))
    ...
    async def matrix_atroom_pl_0(self, matrix_room_id):
        ...
        return await self.opsdroid.send(MatrixPowerLevels(power_levels, target=matrix_room_id))
